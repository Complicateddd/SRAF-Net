...
def coords_fmap2orig(feature,stride):
    ...
    shift_x = torch.reshape(shift_x, [-1])-0.75
    shift_y = torch.reshape(shift_y, [-1])-0.75
...
class GenTargets(nn.Module):
    ...
    def _gen_level_targets(self,out,gt_boxes,classes,stride,limit_range,sample_radiu_ratio=2.5):
        '''
        Args  
...
#        m=gt_boxes.shape[1]
        ...
        off_min=torch.min(ltrb_off,dim=-1)[0]#[batch_size,h*w,m] #if [0]value,[1] index
        off_max=torch.max(ltrb_off,dim=-1)[0]#[batch_size,h*w,m]

        mask_in_gtboxes=off_min>0#:满足最小值大于0的置为1  #[batch_size,h*w,m]
        mask_in_level=(off_max>limit_range[0])&(off_max<=limit_range[1])   #:满足最大值在range里的置为1  #[batch_size,h*w,m]
        ...
        # print(areas_min_ind.shape)
        mm=torch.zeros_like(areas,dtype=torch.long).scatter_(-1,areas_min_ind.unsqueeze(dim=-1),1)>0
        # print(mm.shape)
        reg_targets=ltrb_off[mm]

        # print(temp.shape)
        # reg_targets=ltrb_off[torch.zeros_like(areas,dtype=torch.long).scatter(-1,areas_min_ind.unsqueeze(dim=-1),1)]
        ...
        jj=torch.zeros_like(areas,dtype=torch.long).scatter_(-1,areas_min_ind.unsqueeze(dim=-1),1)>0
        cls_targets=classes[jj]
        ...
        # print(reg_targets.shape,h_mul_w)
...
def compute_cls_loss(preds,targets,mask,s_imform):
    '''
    Args  
    ...
    for batch_index in range(batch_size):
        ...
        ###smooth label
        #index=(target_pos>0).squeeze(-1)
        target_pos=(torch.arange(1,class_num+1,device=target_pos.device)[None,:]==target_pos).float()
        #target_pos[index]=target_pos[index]*0.95+0.05/10
        # print(target_pos)
        # target_pos=(1-0.01)*target_pos+0.01/10
        # print(target_pos)
        # print(target_pos.shape)
        s_imform_tmp=s_imform[batch_index]
        #s_imform_tmp=s_imform*0.95+0.05/10
        #print(s_imform_tmp.shape)
        loss.append(focal_loss_from_logits(pred_pos,target_pos,s_imform_tmp).view(1))
...
def focal_loss_from_logits(preds,targets,s_imform,gamma=2.0,alpha=0.25):
    '''
    Args:
    preds: [n,class_num] 
    targets: [n,class_num]
    ...
    ##to
    # print(preds.shape)
    # print(s_imform.shape)
    x_s=torch.sigmoid(s_imform)
    a = torch.zeros_like(x_s)
    b = torch.ones_like(x_s)
    x=torch.where(x_s>0.4,b,a)
    
    pred_sig=preds.sigmoid()
    # predss=pred_sig*0.95+0.05*x
    predss=pred_sig*0.5+0.5*x
    pt=predss*targets+(1.0-predss)*(1.0-targets)
...
class LOSS(nn.Module):
    ...
    def forward(self,inputs):
        ...
        cls_loss=compute_cls_loss(cls_logits,cls_targets,mask_pos,s_imform).mean()#[]
...
def focal_loss_with_scene(preds,targets,gamma=2.0,alpha=0.1):
    '''
    Args:
    preds: [n,class_num] 
    targets: [n,class_num]
